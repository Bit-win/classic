...
#define LOG_TAG "ov08f10"

#include "c3z01_ov08f10_frontsyx.h"
 ...
 *============================================================================*/
static void ov08f10_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08f10_drv_write_gain(cmr_handle handle,
    ...
    if (aec_info->again->size) {
        ...
        aec_info->again->settings[0].reg_value = 0x01; //page1
        aec_info->again->settings[1].reg_value = gain & 0xff;
		aec_info->again->settings[2].reg_value = 0x01; // trigger new gain
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08f10_drv_write_frame_length(cmr_handle handle,
                                           struct sensor_aec_i2c_tag *aec_info,
                                           cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    ...
    if (aec_info->frame_length->size) {
        /*TODO*/
		// ov08f modify the framelength,insert dummy lines with Vblank registers
		// P1:0x05,P1:0x06, unit of the register value is half row.
        aec_info->frame_length->settings[0].reg_value = 0x01; //page1
        aec_info->frame_length->settings[1].reg_value = ((frame_len - 2501)*2 >> 8) & 0xff;
        aec_info->frame_length->settings[2].reg_value = (frame_len - 2501)*2 & 0xff;
		aec_info->frame_length->settings[3].reg_value = 0x01; //trigger new vblank
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov08f10_drv_write_shutter(cmr_handle handle,
                                      struct sensor_aec_i2c_tag *aec_info,
                                      cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    ...
    if (aec_info->shutter->size) {
        /*TODO*/
		// the unit of exposure register(p1:0x02,p1:0x03,p1:0x04) value is half of Tline.
        aec_info->shutter->settings[0].reg_value = 0x01;//page1
        aec_info->shutter->settings[1].reg_value = (shutter*2 >> 16) & 0xff;
        aec_info->shutter->settings[2].reg_value = (shutter*2 >> 8) & 0xff;
        aec_info->shutter->settings[3].reg_value = shutter*2 & 0xff;
		aec_info->shutter->settings[4].reg_value = 0x01;//trigger new exposure
        /*END*/
    }
}
 ...
 *============================================================================*/
static void ov08f10_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        ov08f10_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    ov08f10_drv_write_shutter(handle, aec_info, shutter);
}

static void ov08f10_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                  struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    ov08f10_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_ov08f10_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_ov08f10_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int ov08f10_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        ov08f10_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_ov08f10_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_ov08f10_mipi_raw_info.sensor_version_info;
...
static cmr_int ov08f10_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static const cmr_u16 ov08f10_pd_is_right[] = {0, 1, 1, 0};
static const cmr_u16 ov08f10_pd_row[] = {2, 6, 10, 14}; //{14, 10, 6, 2};//y

static const cmr_u16 ov08f10_pd_col[] = {10, 10, 2, 2}; //{2, 2, 10, 10};//x
static const struct pd_pos_info ov08f10_pd_pos_r[] = {
    {10, 6}, {2, 10},
};

static const struct pd_pos_info ov08f10_pd_pos_l[] = {
    {10, 2}, {2, 14},
};
...
static const cmr_u16 ov08f10_pd_is_right[] = {0, 1, 0, 1};

static const cmr_u16 ov08f10_pd_row[] = {2, 2, 6, 6};

static const cmr_u16 ov08f10_pd_col[] = {5, 6, 1, 2};
static const struct pd_pos_info ov08f10_pd_pos_l[] = {
    {5, 2}, {1, 6},
};
...
#endif
static const cmr_u32 ov08f10_pd_sns_mode[] = {0, 0, 0, 1};
#define PDAF_BLOCK_SIZE_W 16
#define PDAF_BLOCK_SIZE_H 16

struct sensor_pdaf_info ov08f10_pdaf_info_array[] = {
    ...
    {
	.pd_is_right = (cmr_u16 *)ov08f10_pd_is_right,
	.pd_pos_row = (cmr_u16 *)ov08f10_pd_row,
	.pd_pos_col = (cmr_u16 *)ov08f10_pd_col,
	.pd_pos_r = (struct pd_pos_info *)ov08f10_pd_pos_r,
	.pd_pos_l = (struct pd_pos_info *)ov08f10_pd_pos_l,
	.pd_pos_size = NUMBER_OF_ARRAY(ov08f10_pd_pos_r),
	.pd_offset_x = 0,
	.pd_offset_y = 0,
...
static struct sensor_pdaf_info_tab ov08f10_pdaf_info_tab = {
    .sns_mode_num = NUMBER_OF_ARRAY(ov08f10_pd_sns_mode),
	.pd_sns_mode = ov08f10_pd_sns_mode,
	.pdaf_init_index = 3,
	.pd_init_size_w = SNAPSHOT_WIDTH,
	.pd_init_size_h = SNAPSHOT_HEIGHT,
	.pd_info_tab = ov08f10_pdaf_info_array,
};

static cmr_int ov08f10_drv_get_pdaf_info(cmr_handle handle, void *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    pdaf_info_tab->sns_mode_num = ov08f10_pdaf_info_tab.sns_mode_num;
    pdaf_info_tab->pd_sns_mode = ov08f10_pdaf_info_tab.pd_sns_mode;
    pdaf_info_tab->pdaf_init_index = ov08f10_pdaf_info_tab.pdaf_init_index;
    pdaf_info_tab->pd_init_size_w = ov08f10_pdaf_info_tab.pd_init_size_w;
    pdaf_info_tab->pd_init_size_h = ov08f10_pdaf_info_tab.pd_init_size_h;
    pdaf_info_tab->pd_info_tab = ov08f10_pdaf_info_tab.pd_info_tab;

    pd_info_num = ov08f10_pdaf_info_tab.sns_mode_num;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGI("sensor ov08f10: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ov08f10_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = ov08f10_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        ret = ov08f10_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov08f10_PID_ADDR);

    if (ov08f10_PID_VALUE == pid_value) {
        ver_value =
            hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov08f10_VER_ADDR);
        SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value,
                    ver_value);
        if (ov08f10_VER_VALUE == ver_value) {
            SENSOR_LOGI("this is ov08f10 sensor");
 ...
 *============================================================================*/
static cmr_int ov08f10_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    ov08f10_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                              &ov08f10_aec_info);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.frame_length);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    ov08f10_drv_write_gain(handle, &ov08f10_aec_info, cap_gain);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.again);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int ov08f10_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    ov08f10_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                              &ov08f10_aec_info);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.frame_length);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int ov08f10_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    ov08f10_drv_calc_gain(handle, param, &ov08f10_aec_info);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.again);
    ov08f10_drv_write_reg2sensor(handle, ov08f10_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int ov08f10_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &ov08f10_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    ov08f10_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                              &ov08f10_aec_info);
    ov08f10_drv_calc_gain(handle, info->gain, &ov08f10_aec_info);

    return ret_value;
}

static cmr_int ov08f10_drv_set_master_FrameSync(cmr_handle handle,
                                                cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    if (!strcmp(value2, "1")) {
        ...
        ov08f10_drv_set_master_FrameSync(handle, param);
#endif
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov08f10_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int
ov08f10_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    ov08f10_drv_write_frame_length(
        sns_drv_cxt, &ov08f10_aec_info,
        sns_drv_cxt->sensor_ev_info.preview_framelength);
    ov08f10_drv_write_gain(sns_drv_cxt, &ov08f10_aec_info,
                           sns_drv_cxt->sensor_ev_info.preview_gain);
    ov08f10_drv_write_shutter(sns_drv_cxt, &ov08f10_aec_info,
                              sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt,
                                    ARRAY_SIZE(s_ov08f10_module_info_tab),
                                    s_ov08f10_module_info_tab);
    sensor_ic_set_match_resolution_info(
        sns_drv_cxt, ARRAY_SIZE(s_ov08f10_resolution_tab_raw),
        s_ov08f10_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt,
                                  ARRAY_SIZE(s_ov08f10_resolution_trim_tab),
                                  s_ov08f10_resolution_trim_tab);
    sensor_ic_set_match_static_info(
        sns_drv_cxt, ARRAY_SIZE(s_ov08f10_static_info), s_ov08f10_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,
                                 ARRAY_SIZE(s_ov08f10_mode_fps_info),
                                 s_ov08f10_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    ov08f10_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int ov08f10_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
{
     return &g_ov08f10_mipi_raw_info;
}

static cmr_int ov08f10_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                            void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = ov08f10_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_ov08f10_ops_tab = {
    .create_handle = ov08f10_drv_handle_create,
    .delete_handle = ov08f10_drv_handle_delete,
    .get_data = ov08f10_drv_get_private_data,
    /*---------------------------------------*/
    .power = ov08f10_drv_power_on,
    .identify = ov08f10_drv_identify,
    .ex_write_exp = ov08f10_drv_write_exposure,
    .write_gain_value = ov08f10_drv_write_gain_value,

#if defined(CONFIG_DUAL_MODULE)
    .read_aec_info = ov08f10_drv_read_aec_info,
#endif

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ov08f10_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = ov08f10_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = ov08f10_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = ov08f10_drv_access_val,
    }};
