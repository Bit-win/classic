...
#define LOG_TAG "c3z01_ov32d40_backxl"

#include "c3z01_ov32d40_backxl.h"
...
#define EEPROM_I2C_ADDR_I 0xa2
#define READ_OTP_SIZE 10 //read 10 bytes
#define TRULY_VENDOR_ID_I 0x09
#define TRULY_VCM_ID_I 0xF5
#define TRULY_DRIVERIC_ID_I 0x3B
...
static cmr_int get_vendor_id(cmr_handle handle) {
    ...
    rtn = hw_sensor_read_i2c(handle, EEPROM_I2C_ADDR_I >> 1, otp_data_buf,
...
static cmr_int ov32d40_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if ((otp_data_buf[0] == TRULY_VENDOR_ID_I) && (otp_data_buf[8] == TRULY_VCM_ID_I) && (otp_data_buf[9] == TRULY_DRIVERIC_ID_I)) {
        SENSOR_LOGI("This is truly 1st module c3z01_ov32d40_backxl");
    } else {
        return SENSOR_FAIL;
    }
#endif
...
static cmr_int ov32d40_drv_stream_off(cmr_handle handle, cmr_uint param) {
    ...
    usleep(22*1000); //sensor FAE suggestions
    SENSOR_LOGI("X");
    pthread_mutex_unlock(&ov32d40_sensor_mutex);
    return SENSOR_SUCCESS;
}
