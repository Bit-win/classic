...
#define LOG_TAG "c3z02_ov32d40_backxl"

#include "c3z02_ov32d40_backxl.h"
...
static cmr_int ov32d40_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    SENSOR_AVDD_VAL_E motor_avdd_val = SENSOR_AVDD_2800MV;
...
#define ENABLE_DISTIGUISH_MODULE 1
#if ENABLE_DISTIGUISH_MODULE
#define VENDOR_ID_OTP_INFO_OFFSET 0x0001 //Read otp info from vendor id
#define EEPROM_I2C_ADDR_II 0xa0
#define READ_OTP_SIZE 10 //read 10 bytes
#define TRULY_VENDOR_ID_II 0x09
#define TRULY_VCM_ID_II 0x01
#define TRULY_DRIVERIC_ID_II 0x01

cmr_u8 otp_data_buf[READ_OTP_SIZE]= {0x00};

static cmr_int get_vendor_id(cmr_handle handle) {
    cmr_int rtn = SENSOR_SUCCESS;

    otp_data_buf[0] = (VENDOR_ID_OTP_INFO_OFFSET >>8) & 0xff;
    otp_data_buf[1] = VENDOR_ID_OTP_INFO_OFFSET & 0xff;

    rtn = hw_sensor_read_i2c(handle, EEPROM_I2C_ADDR_II >> 1, otp_data_buf,
                       READ_OTP_SIZE << 16 | SENSOR_I2C_REG_16BIT);

    SENSOR_LOGI("otp_data_buf[0] = 0x%x,otp_data_buf[8] =0x%x,otp_data_buf[9] =0x%x\n", otp_data_buf[0],otp_data_buf[8],otp_data_buf[9]);
...
static cmr_int ov32d40_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if ((otp_data_buf[0] == TRULY_VENDOR_ID_II) && (otp_data_buf[8] == TRULY_VCM_ID_II) && (otp_data_buf[9] == TRULY_DRIVERIC_ID_II)) {
        SENSOR_LOGI("This is truly 2st module c3z02_ov32d40_backxl");
    } else {
        return SENSOR_FAIL;
    }
#endif
