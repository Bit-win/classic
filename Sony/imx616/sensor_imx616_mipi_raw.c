...
/*==============================================================================
 * Description:
 * set quad bayer coding sensitivity correction gain
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_set_qsc_data(cmr_handle handle){
    ...
    for (int i = 0; i < IMX616_QSC_LENGTH; i++) {
        hw_sensor_write_reg(sns_drv_cxt -> hw_handle, 0xC500 + i, *(param_ptr + 0x07D2 + i));
        SENSOR_LOGV("imx616_qsc address {0x%x, 0x%x}", 0xC500 + i, *(param_ptr + 0x07D2 + i));
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_set_awb_gain(cmr_handle handle, void* awb_param){
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int imx616_drv_write_awb_gain(cmr_handle handle){
...
static cmr_int imx616_drv_dump_trace(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u16 value = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int imx616_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    case SENSOR_VAL_TYPE_4IN1_WRITE_AWB_GAIN:
        ret = imx616_drv_set_awb_gain(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_DUMP_TRACE:
        ret = imx616_drv_dump_trace(handle, param_ptr->pval);
        break;
    default:
        break;
    }
...
static cmr_int imx616_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    imx616_drv_write_awb_gain(handle);
snapshot_info:
...
//		imx616_drv_set_xtalk_data(handle, param);
    if (sensor_mode > 2)
        imx616_drv_set_qsc_data(handle);
    else
