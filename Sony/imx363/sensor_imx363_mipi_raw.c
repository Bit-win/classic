...
static cmr_u8 imx363_snspid_is_init = 0;

static cmr_int imx363_drv_save_snspid(cmr_handle handle) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    cmr_u8 snspid[32] = {0};
...
static cmr_int imx363_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if (imx363_PID_VALUE == pid_value || 0x2 == pid_value) {
        ...
        if (!imx363_snspid_is_init) {
            imx363_drv_save_snspid(handle);
            }
        } else {
