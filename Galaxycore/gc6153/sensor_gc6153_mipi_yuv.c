                                                  ...
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
	...
	if (cur_fr_len > shutter) {
	 fps = (float) (const_fps / (cur_fr_len * sns_drv_cxt->trim_tab_info[mode].line_time));
	} else {
	 fps = (float) (const_fps / ((shutter + dummy_line) * sns_drv_cxt->trim_tab_info[mode].line_time));
	}
...
static cmr_int gc6153_drv_identify(cmr_handle handle, cmr_uint param)
{
    ...
    if (GC6153_PID_VALUE == pid_valueH) {
        ...
        if (GC6153_VER_VALUE == pid_valueL) {
            ...
            SENSOR_LOGI("sensor identify fail, pid_valueH = %x, pid_valueL = %x",
