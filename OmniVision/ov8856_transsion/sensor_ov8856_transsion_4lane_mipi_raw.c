...
//#define LOG_TAG "ov8856_transsion_4lane"

#include "sensor_ov8856_transsion_4lane_mipi_raw.h"
cmr_int g_read_awb_flag = 0;/*0,no read otp in eng mode;1,read otp in eng mode,gang.zhang@transsion.com,2021-03-18*/
...
static void ov8856_transsion_drv_write_reg2sensor(cmr_handle handle,
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_transsion_drv_write_gain(cmr_handle handle,
    ...
    if (aec_info->again->size) {
        /*TODO*/
        aec_info->again->settings[0].reg_value = ((cmr_u16)gain_a >> 8) & 0x1f;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_transsion_drv_write_frame_length(cmr_handle handle,
                                          struct sensor_aec_i2c_tag *aec_info,
                                          cmr_u32 frame_len) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov8856_transsion_drv_write_shutter(cmr_handle handle,
                                     struct sensor_aec_i2c_tag *aec_info,
                                     cmr_u32 shutter) {
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
 ...
 *============================================================================*/
static void ov8856_transsion_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
    ...
    if (dest_fr_len != cur_fr_len) {
        ...
        ov8856_transsion_drv_write_frame_length(handle, aec_info, dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
    ov8856_transsion_drv_write_shutter(handle, aec_info, shutter);
...
static void ov8856_transsion_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    ov8856_transsion_drv_write_gain(handle, aec_info, sensor_gain);
}
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    ...
    BOOLEAN power_down = g_ov8856_transsion_shine_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_ov8856_transsion_shine_mipi_raw_info.reset_pulse_level;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int ov8856_transsion_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        ov8856_transsion_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_ov8856_transsion_shine_mipi_raw_info.name;
    ex_info->sensor_version_info =
        (cmr_s8 *)g_ov8856_transsion_shine_mipi_raw_info.sensor_version_info;
...
static cmr_int ov8856_transsion_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static cmr_int ov8856_transsion_drv_set_raw_info(cmr_handle handle, cmr_u8 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    SENSOR_LOGV("sensor ov8856_transsion: param_ptr->type=%x", param_ptr->type);

    switch (param_ptr->type) {
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ov8856_transsion_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = ov8856_transsion_drv_get_fps_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
        ret = sns_drv_cxt->is_sensor_close = 1;
        break;
    case SENSOR_VAL_TYPE_GET_PDAF_INFO:
        // ret = ov8856_transsion_drv_get_pdaf_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_SET_RAW_INFOR:
        ov8856_transsion_drv_set_raw_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_READ_OTP:
        ov8856_transsion_update_otp(handle, param_ptr->pval);
        break;
    default:
        break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov8856_transsion_PID_ADDR);
    pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, (ov8856_transsion_PID_ADDR + 1)) + (pid_value <<8);

    if (ov8856_transsion_PID_VALUE != pid_value)
    {
    ...
    if(otp_read_once == 0){
        ov8856_transsion_read_otp(handle,param_ptr);
        otp_read_once++ ;
    }
...
static cmr_int ov8856_transsion_drv_ioctl(cmr_handle handle, int cmd, void *param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    //struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static cmr_int ov8856_transsion_drv_before_snapshot(cmr_handle handle, cmr_uint param) {
    ...
    ov8856_transsion_drv_calc_exposure(handle, cap_shutter, 0, capture_mode,
                             &ov8856_transsion_aec_info);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.frame_length);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.shutter);

    sns_drv_cxt->sensor_ev_info.preview_gain = cap_gain;
    ov8856_transsion_drv_write_gain(handle, &ov8856_transsion_aec_info, cap_gain);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.again);
    //ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int ov8856_transsion_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    ov8856_transsion_drv_calc_exposure(handle, exposure_line, dummy_line, size_index,
                             &ov8856_transsion_aec_info);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.frame_length);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int ov8856_transsion_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;

    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    ov8856_transsion_drv_calc_gain(handle, param, &ov8856_transsion_aec_info);
    ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.again);
    //ov8856_transsion_drv_write_reg2sensor(handle, ov8856_transsion_aec_info.dgain);
    return ret_value;
}
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int ov8856_transsion_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &ov8856_transsion_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    ov8856_transsion_drv_calc_exposure(handle, exposure_line, dummy_line, mode,
                             &ov8856_transsion_aec_info);
    ov8856_transsion_drv_calc_gain(handle, info->gain, &ov8856_transsion_aec_info);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ov8856_transsion_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
    ...
    usleep(STREAMOFF_TIMEOUT*1000);

    SENSOR_LOGI("X");
    return SENSOR_SUCCESS;
}

static cmr_int ov8856_transsion_drv_handle_create(struct sensor_ic_drv_init_para *init_param,cmr_handle *sns_ic_drv_handle)
{
    ...
    ov8856_transsion_drv_write_frame_length(sns_drv_cxt, &ov8856_transsion_aec_info,sns_drv_cxt->sensor_ev_info.preview_framelength);
    ov8856_transsion_drv_write_gain(sns_drv_cxt, &ov8856_transsion_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
    ov8856_transsion_drv_write_shutter(sns_drv_cxt, &ov8856_transsion_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_ov8856_transsion_module_info_tab), s_ov8856_transsion_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_ov8856_transsion_resolution_tab_raw), s_ov8856_transsion_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_ov8856_transsion_resolution_trim_tab), s_ov8856_transsion_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_ov8856_transsion_static_info), s_ov8856_transsion_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt,ARRAY_SIZE(s_ov8856_transsion_mode_fps_info),s_ov8856_transsion_mode_fps_info);
    /*sensor_ic_set_match_fov_info(
        sns_drv_cxt, ARRAY_SIZE(s_ov8856_transsion_fov_info), s_ov8856_transsion_fov_info, (void*)&s_ov8856_transsion_exif_info);*/

    /*init exif info,this will be deleted in the future*/
    ov8856_transsion_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int ov8856_transsion_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int ov8856_transsion_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
{
     return &g_ov8856_transsion_shine_mipi_raw_info;
}
/*==============================================================================
 * Description:
 * all ioctl functoins
 ...
 * .power = ov8856_transsion_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_ov8856_transsion_ops_tab = {
    .create_handle = ov8856_transsion_drv_handle_create,
    .delete_handle = ov8856_transsion_drv_handle_delete,
    .get_data = ov8856_transsion_drv_get_private_data,
    /*---------------------------------------*/
    .power = ov8856_transsion_drv_power_on,
    .identify = ov8856_transsion_drv_identify,
    .ex_write_exp = ov8856_transsion_drv_write_exposure,
    .write_gain_value = ov8856_transsion_drv_write_gain_value,
    .ioctl = ov8856_transsion_drv_ioctl,
    .read_aec_info = ov8856_transsion_drv_read_aec_info,

    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ov8856_transsion_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = ov8856_transsion_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = ov8856_transsion_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = ov8856_transsion_drv_access_val,
    }};
