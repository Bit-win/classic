...
static void ov5675_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
                                     cmr_u32 dummy_line, cmr_u16 mode,cmr_u64 exp_time,
                                     struct sensor_aec_i2c_tag *aec_info) {
    cmr_u32 dest_fr_len = 0;
    cmr_u32 fr_len = 0;
    float fps = 0.0;
    cmr_u32 shutter_reg = 0;
    ...
    SENSOR_LOGD("mode = %d, exposure_line = %d, dummy_line= %d, fps = %f",
    ...
    shutter_reg = shutter >> 1;
    if (aec_info->shutter->size) {
        aec_info->shutter->settings[0].reg_value = (shutter_reg << 0x04) & 0xff;
        aec_info->shutter->settings[1].reg_value = (shutter_reg >> 0x04) & 0xff;
        aec_info->shutter->settings[2].reg_value = (shutter_reg >> 0x0c) & 0x0f;
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
}

static void ov5675_drv_calc_gain(cmr_handle handle, cmr_uint isp_gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	cmr_u32 isp_gain_tmp = 0;
    cmr_u32 sensor_gain = 0;

    isp_gain_tmp = isp_gain > ISP_BASE_GAIN ? isp_gain : ISP_BASE_GAIN ;
    sensor_gain = isp_gain_tmp * SENSOR_BASE_GAIN / ISP_BASE_GAIN;

    SENSOR_LOGD("isp_gain = 0x%x, sensor_gain = 0x%x", isp_gain,
                sensor_gain);
    ...
    if (aec_info->again->size) {
        aec_info->again->settings[0].reg_value = (sensor_gain >> 8) & 0x1f;
        aec_info->again->settings[1].reg_value = sensor_gain & 0xff;
    }
...
static cmr_int ov5675_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    ...
    ov5675_drv_calc_exposure(handle, exposure_line, dummy_line, mode, exp_time,
                                &ov5675_aec_info);
    ov5675_drv_calc_gain(handle, info->gain, &ov5675_aec_info);
