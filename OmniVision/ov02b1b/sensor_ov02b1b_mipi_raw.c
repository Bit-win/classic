...
static void ov02b1b_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    ...
    if(aec_info->frame_length->size){
        aec_info->frame_length->settings[1].reg_value = ((frame_len-1220) >> 8) & 0x7f;
                                                  ...
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
    ...
    ov02b1b_drv_write_frame_length(handle, aec_info,dest_fr_len);
    }
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
...
static cmr_u8 ov02b1b_snspid_is_init = 0;

static cmr_int ov02b1b_drv_save_snspid(cmr_handle handle) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    cmr_u8 snspid[32] = {0};
...
static cmr_int ov02b1b_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	if (ov02b1b_PID_VALUE == pid_value) {
		...
		if (ov02b1b_VER_VALUE == ver_value) {
            ...
            if (!ov02b1b_snspid_is_init) {
                ov02b1b_drv_save_snspid(handle);
            }
		} else {
...
static cmr_int ov02b1b_drv_stream_on(cmr_handle handle, cmr_uint param)
{
    ...
    /*SPRD recommends using sw framesync*/
#if 0
