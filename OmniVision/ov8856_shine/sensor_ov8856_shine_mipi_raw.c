...
static cmr_int ov8856_drv_dump_trace(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u16 value1 = 0, value2 = 0, value3 = 0, value4 = 0, value5 = 0, value6 = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    value1 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x0100);
    value2 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3500);
    value3 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3501);
    value4 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x3502);
    value5 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x380e);
    value6 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x380f);

    SENSOR_LOGI("stream-reg, 0x0100:0x%x", value1);
    SENSOR_LOGI("shutter-reg,  0x3500:0x%x, 0x3501:0x%x, 0x3502:0x%x", value2, value3, value4);
    SENSOR_LOGI("frame-reg,  0x380e:0x%x, 0x380f:0x%x", value5, value6);
...
static cmr_int ov8856_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    case SENSOR_VAL_TYPE_DUMP_TRACE:
        ret = ov8856_drv_dump_trace(handle, param_ptr->pval);
        break;
    default:
        break;
    }
...
static cmr_u8 ov8856_shine_snspid_is_init = 0;

static cmr_int ov8856_drv_save_snspid(cmr_handle handle) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    cmr_u8 snspid[32] = {0};
...
static cmr_int ov8856_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    if (ov8856_PID_VALUE == pid_value) {
        ...
        if (!ov8856_shine_snspid_is_init) {
             ov8856_drv_save_snspid(handle);
        }
        } else {
