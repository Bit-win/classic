...
#define MAX(a, b) (a) > (b) ? (a) : (b)
    ...
    SENSOR_LOGD("shutter=%d, dummy_line=%d,dest_fr_len=%d",
                shutter, dummy_line, dest_fr_len);
...
static cmr_u8 ov5675_dual_snspid_is_init = 0;
...
static cmr_int ov5675_drv_stream_on(cmr_handle handle, cmr_uint param) {
    ...
    if (!strcmp(value2, "1")) {
        if ((2 == sns_drv_cxt->sensor_id) && (MODE_BOKEH == sns_drv_cxt->is_multi_mode ||
            MODE_PORTRAIT == sns_drv_cxt->is_multi_mode)) {
...
static cmr_int ov5675_back_drv_dump_trace(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    cmr_u16 value1 = 0, value2 = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;

    value1 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x4848);
    value2 = hw_sensor_read_reg(sns_drv_cxt->hw_handle, 0x4849);
    SENSOR_LOGI("FRM_CNT 0x4848:0x%x 0x4849:0x%x", value1, value2);
...
static cmr_int ov5675_drv_access_val(cmr_handle handle, unsigned long param) {
    ...
    case SENSOR_VAL_TYPE_DUMP_TRACE:
        rtn = ov5675_back_drv_dump_trace(handle, param_ptr->pval);
        break;
    default:
        break;
    }
