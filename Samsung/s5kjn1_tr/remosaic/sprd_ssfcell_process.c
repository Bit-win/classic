...
int ss4c_init(ssfcell_init init){
    ...
    ret = remosaic_init(init.width, init.height,init.bayer_order, init.pedestal);
    if(ret != RET_OK)
    {
    return 0;
    }
...
int ss4c_process(unsigned short *src,unsigned short *dst,
...
#if 1
    int imgsize = (input_width * 5 / 4 + 8) * input_height;
 ...
 remosaic_process_gain_set(g_gain);

 remosaic_process(src, imgsize, dst, imgsize1);
