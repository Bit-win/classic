/*
 * This file is modified by Unisoc (Shanghai) Technologies Co., Ltd 2023
 */
...
int ss4c_process(unsigned short * src,unsigned short * dst, 
				struct st_remosaic_param* p_param, struct st_remosaic_gain* g_gain, uint32_t input_width, uint32_t input_height) {
    // Step 3: Process Fcell.
 //   struct timeval start, end;
#if 1
    int imgsize = (input_width * 5 / 4) * input_height;
    int imgsize1 = input_width * input_height * 2;
 ...
 //remosaic_process_param_set(g_gain);

 remosaic_process((uint8_t *)src, imgsize, (uint8_t *)dst, imgsize1);
