...
#define LOG_TAG "s5k3p9sx04_ts_mipi_raw"
#include "sensor_s5k3p9sx04_ts_mipi_raw.h"
...
static cmr_u8 s5k3p9sx04_snspid_is_init = 0;

static cmr_int s5k3p9sx04_drv_save_snspid(cmr_handle handle) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int s5k3p9sx04_drv_identify(cmr_handle handle, cmr_int param) {
    ...
    if (s5k3p9sx04_PID_VALUE == pid_value) {
        ...
        if (s5k3p9sx04_VER_VALUE == ver_value) {
            ...
            if (!s5k3p9sx04_snspid_is_init) {
             s5k3p9sx04_drv_save_snspid(handle);
            }
        } else {
            SENSOR_LOGI("SENSOR_s5k3p9sx04: Identify this is %x%x sensor !",
                        pid_value, ver_value);
            return ret_value;
        }
    } else {
...
static SENSOR_REG_T s5k3p9sx04_grp_hold_start_reg[] = {

};

static struct sensor_i2c_reg_tab s5k3p9sx04_grp_hold_start_tab = {
    .settings = s5k3p9sx04_grp_hold_start_reg,
    .size = ARRAY_SIZE(s5k3p9sx04_grp_hold_start_reg),
};

static SENSOR_REG_T s5k3p9sx04_grp_hold_end_reg[] = {

};

static struct sensor_i2c_reg_tab s5k3p9sx04_grp_hold_end_tab = {
    .settings = s5k3p9sx04_grp_hold_end_reg,
    .size = ARRAY_SIZE(s5k3p9sx04_grp_hold_end_reg),
};
...
static struct sensor_aec_i2c_tag s5k3p9sx04_aec_info = {
    ...
    .grp_hold_start = &s5k3p9sx04_grp_hold_start_tab,
    .grp_hold_end = &s5k3p9sx04_grp_hold_end_tab,
};
